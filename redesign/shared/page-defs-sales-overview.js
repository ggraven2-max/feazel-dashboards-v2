...
(function () {
  ...
  pages.markets = {
    ...
    sections: [
      ...
      // Source can be either the Salesforce aggregated XLSX (current) or the
      // legacy per-opportunity export. Calculator normalizes both to the
      // same rows shape: { branch, opps, soldAmt, closePct, nsli }.
      ...
      var netClose = cb.totals.closePct || 0;
      var netNsli  = cb.totals.nsli || 0;

      return [
            { kind: 'prose',
              heading: 'Closing percentage and NSLI by branch',
              caption: 'Same branches, second lens: how efficiently each market converts the leads it gets into signed dollars. Layout mirrors the Salesforce Closing Percent By Branch report.'
            },
            {
              kind: 'kpi-row', cols: 4,
              items: [
                { label: 'Network Closing %', value: netClose.toFixed(1) + '%',
                  sub: cb.totals.opps.toLocaleString() + ' total opportunities (Record Count)', tone: 'navy' },
                { label: 'Network NSLI',      value: '$' + netNsli.toLocaleString(),
                  sub: 'Net Sales per Lead Issued · weighted across all branches', tone: 'navy' },
            ...
            },
            {
              ...
              charts: [
                {
                  title: 'Closing Percent by Branch',
                  sub: 'Network avg ' + netClose.toFixed(1) + '% · green ≥40%, red <25%',
                  height: 360,
                  config: {
                    type: 'bar',
                    data: {
                      ...
                      datasets: [{
                        ...
                        backgroundColor: cb.rows.map(function (r) {
                          return r.closePct >= 40 ? pal.success : r.closePct < 25 ? pal.danger : pal.navy;
                        }),
                        label: 'Closing %'
                      }]
                    },
                ...
                {
                  title: 'NSLI by Branch',
                  sub: 'Network avg $' + netNsli.toLocaleString() + ' · green ≥+25%, red ≤−25%',
                  height: 360,
                  config: {
                    type: 'bar',
                    data: {
                      ...
                      datasets: [{
                        ...
                        backgroundColor: cb.rows.map(function (r) {
                          return r.nsli >= netNsli * 1.25 ? pal.success
                               : r.nsli < netNsli * 0.75 ? pal.danger : pal.navy;
                        }),
                        label: 'NSLI'
                      }]
                    },
            ...
            {
              kind: 'table',
              heading: 'Closing Percent By Branch',
              caption: 'Source: ' + cb.source,
              headers: [
                { label: 'Branch Location to Service', num: false },
                { label: 'Sum of Sold Deals', num: true },
                { label: 'Closing Percent', num: true },
                { label: 'NSLI', num: true },
                { label: 'Record Count', num: true }
              ],
              rows: cb.rows.map(function (r) {
                var closePill = r.closePct >= 40
                ...
                var nsliPill = r.nsli >= netNsli * 1.25
                ...
                { html: '<strong>' + netClose.toFixed(1) + '%</strong>' },
                { html: '<strong>$' + netNsli.toLocaleString() + '</strong>' },
                { html: '<strong>' + cb.totals.opps.toLocaleString() + '</strong>' }
              ]])
            },
            {
              kind: 'callout',
              tone: lowClose.closePct < 20 ? 'danger' : 'warn',
              title: 'Where to focus this week',
              body: '<strong>' + lowClose.branch + '</strong> is at <strong>' + lowClose.closePct.toFixed(1) + '%</strong> closing on ' + lowClose.opps + ' opportunities (NSLI $' + lowClose.nsli.toLocaleString() + '), below the network ' + netClose.toFixed(1) + '% / $' + netNsli.toLocaleString() + '. <strong>' + topClose.branch + '</strong> sets the bar at <strong>' + topClose.closePct.toFixed(1) + '%</strong> closing, and <strong>' + topNsli.branch + '</strong> leads NSLI at <strong>$' + topNsli.nsli.toLocaleString() + '</strong> per lead. Worth a coaching call on lead handling and price defense at the bottom-tier branches before the Q3 push.'
            }
      ];
    })()))
  };
